...
export const KpiGrid: React.FC<KpiGridProps> = ({
  ...
  // Multi-dimensional Reactive KPI Generator (Kecamatan + Year + Sector)
  const getDisplayKpis = (): KpiItem[] => {
    // If a specific Kecamatan is selected
    if (selectedKecData) {
    ...
    // If Sektor is selected
    if (selectedSektor === '1') { // Kesehatan & Gizi
      return [
    ...
    if (selectedSektor === '2') { // Pendidikan & Kebudayaan
      return [
    ...
    if (selectedSektor === '3') { // Pariwisata & Pemuda Olahraga
      return [
    ...
    if (selectedSektor === '4') { // Perekonomian & Investasi
      return [
    ...
    if (selectedSektor === '5') { // Pemberdayaan Desa
      return [
    ...
    if (selectedSektor === '6') { // Infrastruktur
      return [
    ...
    // Default ALL (Responsive to Year)
    return [
      {
        ...
        >
          <div>
            ...
            <div className="flex items-center justify-between gap-2 mb-3 min-w-0">
              ...
              <span className="px-2 py-0.5 text-[10px] font-semibold rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 whitespace-nowrap shrink-0">
                {kpi.change}
              </span>
            </div>
          ...
          {/* Footer Producer Info with Icon */}
