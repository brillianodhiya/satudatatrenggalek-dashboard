...
import { BarChart3, Table, MapPin, Sparkles } from 'lucide-react';
...
export const DashboardSubNav: React.FC<DashboardSubNavProps> = ({
  ...
  return (
    <div className="sticky top-[64px] z-20 backdrop-blur-md bg-[#F3F4F8]/90 dark:bg-[#0B0F19]/90 border-b border-slate-200/80 dark:border-slate-800 py-2.5 mb-6 -mx-3 sm:-mx-6 lg:-mx-8 px-3 sm:px-6 lg:px-8 shadow-2xs transition-all">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-full text-xs sm:text-sm font-bold transition-all whitespace-nowrap cursor-pointer border ${
                isActive
            ...
            >
              <span className={isActive ? 'text-cyan-400 dark:text-cyan-600' : 'text-slate-500 dark:text-slate-400'}>
                {tab.icon}
              </span>
              <span>{tab.label}</span>
              {tab.badge && (
                <span
                  className={`ml-1.5 px-2 py-0.5 text-[10px] font-bold font-mono rounded-full border ${
                    isActive
